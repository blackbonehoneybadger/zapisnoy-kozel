...
Экономика (зёрна / rewards / claims) по умолчанию пишет в
`server/data/doffa-*.json`. Если задан `DATABASE_URL` (или `POSTGRES_URL`),
сервер переключается на Postgres:

```bash
cd server
# DATABASE_URL=postgres://…  (Neon / Replit / любой Postgres)
npm run migrate        # применяет server/db/schema.sql
npm start
```

Без `DATABASE_URL` `npm run migrate` безопасно выходит (Postgres optional).
Флаги по умолчанию: `SOL_STAKES_ENABLED=false`, `DOFFA_CLAIM_ENABLED=false`
(реальный SPL Claim — следующий этап).
